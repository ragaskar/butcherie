...
func newScrollButcherie(results [][]float64) *Butcherie {
	b := &Butcherie{
	...
	close(b.cdp.done) // mark as "closed" so close() won't block in tests
	return b
}

func TestEnsureLoaded_SkipLoadWait(t *testing.T) {
	b := newScrollButcherie(nil)
	ctx := context.Background()
	if err := b.ensureLoaded(ctx, LoadOptions{SkipLoadWait: true}); err != nil {
...
func TestScrollToBottom_MultiPage(t *testing.T) {
	...
	if b.wd.(*stubWD).call != 3 {
		t.Errorf("expected 3 scroll calls, got %d", b.wd.(*stubWD).call)
	}
}
...
func TestEnsureLoaded_InvalidRegex(t *testing.T) {
	b := newScrollButcherie([][]float64{{800, 600}})
	ctx := context.Background()
	err := b.ensureLoaded(ctx, LoadOptions{IgnoreRequests: []string{"[invalid"}})
	if err == nil {
